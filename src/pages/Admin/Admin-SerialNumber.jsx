...
const AdminSerialNumber = () => {
  ...
  const fetchserial_numbers = async () => {
    try {
      const response = await fetch(
        "http://localhost:3000/api/admin/serial-numbers",
      );
  ...
  const handleSubmit = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const { error } = await supabase.from("serial_number").insert(payload);
  ...
  /* ================= VIEW MODAL LOGIC ================= */ const handleView =
    (serial) => {
  ...
  return (
    <div
      className={`dashboard-content ${darkMode ? "dark-mode" : ""}`}
    ...
    >
      ...
      <div className="header-row" style={{ marginBottom: "24px" }}>
        <div>
          ...
          <p className="subtitle" style={{ color: "#777" }}>
            Manage and track all serial numbers
          </p>
        </div>
      </div>
      ...
      >
        <div
          className="card admin-card stats-card"
          style={{
            background: "white",
            padding: "20px",
            borderRadius: "10px",
        ...
        <div
          className="card admin-card stats-card used"
          style={{
            background: "white",
            padding: "20px",
            borderRadius: "10px",
      ...
      >
              ...
              {(() => {
                ...
                return currentRecords.map((item, index) => (
                  ...
                  >
                    ...
                    >
                      {item.is_issued || item.submissionStatus === "Issued"
                        ? "Yes"
                        : "-"}
                    </td>
                    <td
                    ...
                    >
                      {item.Confirm || item.submissionStatus === "Issued"
                        ? "Yes"
                        : "-"}
                    </td>
                    <td
                    ...
                    >
                      {item.issuer ? (
                        ...
                        <span
                          ...
                          onClick={() => navigate("/admin/ManageUsers")}
                        >
                          {item.submissionProfile.first_name}{" "}
                          {item.submissionProfile.last_name}
                        </span>
                      ) : (
                    ...
                    <td style={{ padding: "16px 12px" }}>
                      <span
                        className={`type-badge ${item.serial_type?.replace(/\s+/g, "-").toLowerCase()}`}
                        style={{
                          padding: "4px 10px",
                          borderRadius: "20px",
                          fontSize: "12px",
        ...
        >
          ...
          <div className="pagination-info" style={{ fontSize: "14px" }}>
            <span className="current-page-text" style={{ fontWeight: "600" }}>
              {currentPage}
            </span>
            <span className="total-pages-text">
              {" "}
              of{" "}
              {Math.ceil(
                serial_numbers.filter(
                  (i) => filterType === "All" || i.serial_type === filterType,
                ).length / 16,
              ) || 1}
            </span>
          </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <div className="modal-form">
              ...
              >
                <p>
                  <strong>Serial Number:</strong> {selectedSerial.serial_number}
                </p>
                <p>
                  <strong>Serial Type:</strong> {selectedSerial.serial_type}
                </p>
                <p>
                  ...
                  {selectedSerial.issuer
                    ...
                    : selectedSerial.submissionProfile
                      ...
                      : selectedSerial.RequestedBy ||
                        selectedSerial.ResponseID ||
                        "Not yet taken"}
                </p>
                <p>
                  <strong>Request Date:</strong>{" "}
                  {new Date(selectedSerial.date).toLocaleDateString()}
                </p>
              </div>
              ...
              >
                ...
                >
                  <div className="hierarchy-card">
                    ...
                    <span>
                      {new Date(selectedSerial.date).toLocaleString()}
                    </span>
                  </div>
                  <div className="hierarchy-card">
                    <span>
                      <span
                        style={{
                          color:
                            selectedSerial.is_issued ||
                            selectedSerial.submissionStatus === "Issued"
                              ? "var(--success-color)"
                              : "#ccc",
                          marginRight: "10px",
                        }}
                    ...
                    <span
                      className={`status-badge ${selectedSerial.is_issued || selectedSerial.submissionStatus === "Issued" ? "active" : "inactive"}`}
                    >
                      {selectedSerial.is_issued ||
                      selectedSerial.submissionStatus === "Issued"
                        ? "Confirmed"
                  ...
                  <div className="hierarchy-card">
                    ...
                    <span
                      className={`status-badge ${selectedSerial.ResponseID || selectedSerial.submissionStatus === "Issued" ? "active" : "inactive"}`}
                    >
                      {selectedSerial.ResponseID ||
                      selectedSerial.submissionStatus === "Issued"
                        ? "Completed"
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontSize: "11px", color: "#666" }}>
                        Type: {selectedSerial.file_type || "Unknown"} | Added:{" "}
                        {new Date(
                          selectedSerial.file_added_date || selectedSerial.date,
                        ).toLocaleDateString()}
                      </span>
                    </div>
                    <span
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  border: "none",
                  backgroundColor: darkMode ? "#2563eb" : "#003266",
                  color: "#fff",
                  fontSize: "14px",
                  fontWeight: "600",
