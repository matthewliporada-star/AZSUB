...
const ManageUsers = () => {
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const shouldShow =
            newNotification.user_id === adminId ||
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (ownedUsersError) {
        console.error(
          "Error fetching admin-owned users for notifications:",
          ownedUsersError.message,
        );
        return;
      }
      ...
      if (ownedUserIds.length > 0) {
        query = query.or(
          `user_id.eq.${user.id},target_user_id.in.(${ownedUserIds.join(",")})`,
        );
      } else {
  ...
  const fetchUsers = async (adminIdFromParam) => {
    try {
      ...
      const { data, error } = await supabase
        .from("profiles")
        .select("*")
        .eq("created_by", adminIdFromParam)
        .order("created_at", { ascending: false });
    ...
    async (role, excludeUserId = null) => {
      ...
      if (role === "AP" && adminId && adminMP) {
        ...
        `,
          ...
          console.log(
            `Found ${alProfiles.length} ALs under MP ${adminMP.first_name} ${adminMP.last_name}:`,
            alProfiles,
          );
  ...
  const openViewModal = async (u) => {
    ...
    const [supRes, subRes] = await Promise.all([
      supabase
        .from("user_hierarchy")
        .select("profiles:report_to_id(first_name, last_name, account_type)")
        .eq("user_id", u.id)
        .eq("is_active", true)
        .maybeSingle(),
      supabase
        .from("user_hierarchy")
        .select("profiles:user_id(first_name, last_name, account_type)")
        .eq("report_to_id", u.id)
        .eq("is_active", true),
    ]);

    setViewingSupervisor(supRes.data?.profiles || null);
    setViewingSubordinates(
      subRes.data?.map((d) => d.profiles).filter(Boolean) || [],
    );
  };
  ...
  const submitUser = async (e) => {
    ...
    try {
      ...
      if (!isEditMode && formData.position === "MP" && hasCreatedMP) {
        throw new Error(
          "You have already created an MP. Each admin can only create one Management Partner (MP). ",
        );
      }
              ...
              <div className="password-position-row">
                ...
                <div className="input-group">
                  ...
                  {!hasCreatedMP && !isEditMode && (
                    ...
                    >
                      Please create a Management Partner (MP) first before
                      adding AL or AP.
                    </small>
                  )}
                </div>
              ...
              {formData.position === "AL" && (
                <div className="input-group">
                  ...
                  <input
                    ...
                    value={
                      adminMP
                        ? `${adminMP.first_name} ${adminMP.last_name}`
                        : "(No MP found)"
                    }
                    readOnly
                  />
                  ...
                  >
                    ...
                    {potentialUplines.length === 0 ? (
                      <option value="" disabled>
                        No AL users available. Please create an AL first.
                      </option>
                    ) : (
                ...
                >
                  ...
                  <div className="hierarchy-card">
                    {viewingSupervisor ? (
                      <>
                        <span>
                          {viewingSupervisor.first_name}{" "}
                          {viewingSupervisor.last_name}
                        </span>
