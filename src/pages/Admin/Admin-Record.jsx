...
const AdminRecord = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      if (recordsData && recordsData.length > 0) {
        const policyIds = [
          ...new Set(
            recordsData.map((r) => r.policy_id).filter((id) => id != null),
          ),
        ];
  ...
  const calculateStats = (data) => {
    ...
    const uniqueClients = new Set(
      activeRecords.map((record) => record.client_name),
    ).size;
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

    const recentSubmissions = activeRecords.filter(
      (record) =>
        record.date_submitted &&
        new Date(record.date_submitted) >= thirtyDaysAgo,
    ).length;

    const pendingProcessing = activeRecords.filter(
      (record) => !record.date_processed && !record.is_archived,
    ).length;
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            serial_number: formData.serial_number
              ? parseInt(formData.serial_number)
              : null,
  ...
  const handleArchive = async (id, clientName, currentArchiveStatus) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to ${action} the record for ${clientName}?`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await fetchRecords();
      alert(
        `Record ${currentArchiveStatus ? "restored" : "archived"} successfully!`,
      );
    } catch (err) {
      ...
      <div className="content-container">
        ...
        >
          ...
          <div className="header-actions">
            ...
            >
              <i
                className={`fa-solid ${viewMode === "archived" ? "fa-file-lines" : "fa-archive"}`}
              ></i>
                  ...
                  >
                    ...
                    <td className="submission-cell">
                      {record.submission_type ? (
                        <span
                          className={`submission-badge ${record.submission_type.toLowerCase()}`}
                        >
                          {record.submission_type}
                        </span>
                      ) : (
                    ...
                    <td
                      className="intermediary-cell"
                      title={record.intermediary}
                    >
                      {record.intermediary
                        ? truncateName(record.intermediary, 15)
                        : "—"}
                    </td>
                    <td
                      className="email-cell"
                      title={record.intermediary_email}
                    >
                      {record.intermediary_email
                        ? truncateName(record.intermediary_email, 20)
                        : "—"}
                    </td>
                    ...
                    <td className="date-cell">
                      {formatDate(record.date_processed)}
                    </td>
                    <td className="date-cell">
                      {formatDate(record.date_issued)}
                    </td>
                    ...
                    <td className="actions-cell">
                      <div className="action-buttons">
                        ...
                        <button
                          ...
                          className={
                            record.is_archived ? "restore-btn" : "archive-btn"
                          }
                          title={
                            record.is_archived
                              ? "Restore Record"
                        ...
                        >
                          <i
                            className={`fa-solid ${record.is_archived ? "fa-box-open" : "fa-box-archive"}`}
                          ></i>
                        </button>
                      </div>
                    </td>
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-form-grid">
                ...
                <div className="form-group full-width">
                  <label>
                    Select Policy <span className="required">*</span>
                  </label>
                  <select
                    name="policy_id"
                ...
                {selectedPolicy && (
                  <div className="policy-details full-width">
                    ...
                    <div className="policy-detail-item">
                      ...
                      <span className="detail-value">
                        {selectedPolicy.form_type}
                      </span>
                    </div>
                    <div className="policy-detail-item">
                      <span className="detail-label">Request Type</span>
                      <span className="detail-value">
                        {selectedPolicy.request_type}
                      </span>
                    </div>
                    <div className="policy-detail-item">
