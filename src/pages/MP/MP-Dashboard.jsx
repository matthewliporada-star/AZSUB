...
const MPDashboard = () => {
  ...
  const { mpStats, alPerformance, apPerformance, refreshData, loading, error } =
    useMPData();
  ...
  useEffect(() => {
    if (selectedStat && showStatDetailsModal) {
      fetchStatHistoryData(selectedStat).then((data) =>
  ...
  const calculateStats = () => {
    ...
    const performingALs = safeALPerformance.filter(
      (al) => al.status === "PERFORMING",
    ).length;
    const totalALANP = safeALPerformance.reduce(
      (sum, al) => sum + (al.monthlyANP || 0),
      0,
    );
    const totalALPolicies = safeALPerformance.reduce(
      (sum, al) => sum + (al.monthlyCases || 0),
      0,
    );
    ...
    const totalActivityRatioSum = safeALPerformance.reduce(
      (sum, al) => sum + (al.activityRatio || 0),
      0,
    );
    const avgActivityRatio =
      totalALs > 0 ? totalActivityRatioSum / totalALs : 0;

    const totalAPs = safeAPPerformance.length;

    // Count active APs (those with at least 1 monthly case)
    const activeAPs = safeAPPerformance.filter(
      (ap) => (ap.monthlyCases || 0) > 0,
    ).length;

    const totalAPANP = safeAPPerformance.reduce(
      (sum, ap) => sum + (ap.monthlyANP || 0),
      0,
    );
    const totalAPPolicies = safeAPPerformance.reduce(
      (sum, ap) => sum + (ap.monthlyCases || 0),
      0,
    );
    ...
    const currentMonthlyANP = safeALPerformance.reduce(
      (sum, al) => sum + (al.monthlyANP || 0),
      0,
    );
    const currentMonthlyPolicies = safeALPerformance.reduce(
      (sum, al) => sum + (al.monthlyCases || 0),
      0,
    );
    const currentMonthlyDeclined = safeALPerformance.reduce(
      (sum, al) => sum + (al.monthlyDeclined || 0),
      0,
    );
  ...
  const formatStatTrend = (statType) => {
    if (loadingTrends)
      return { arrow: "", percentage: "...", className: "loading" };
  ...
  const handleViewPolicyDetails = async (al) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/mp/policy-details/${al.id}?year=${appliedFilters.year}`,
      );
      const result = await response.json();
      if (result.success) {
  ...
  const renderStatDetails = () => {
    ...
    return (
      <div>
        ...
        >
          {currentHistoryData.description} - {currentMonth}{" "}
          {appliedFilters.year}
        </p>
        ...
        >
          ...
          >
            <div>
              ...
              >
                {selectedStat === "activityRatio" &&
                  `${currentHistoryData?.currentValue || 0}%`}
                {selectedStat === "totalANP" &&
                  `₱ ${(currentHistoryData?.currentValue || 0).toLocaleString()}`}
                {selectedStat === "monthlyANP" &&
                  `₱ ${(currentHistoryData?.currentValue || 0).toLocaleString()}`}
                {selectedStat === "apAvgANP" &&
                  `₱ ${(currentHistoryData?.currentValue || 0).toLocaleString()}`}
                {selectedStat === "totalCases" && (
                  <>
                    ...
                    >
                      {mpStats.monthlyCases?.toLocaleString() || 0} Issued ·{" "}
                      {mpStats.monthlyDeclined || 0} Declined
                    </div>
                  </>
                )}
                {selectedStat === "totalALs" &&
                  (currentHistoryData?.currentValue || mpStats.totalALs)}
                {selectedStat === "totalAPs" &&
                  (currentHistoryData?.currentValue || mpStats.totalAPs)}
              </div>
            </div>
            <div>
              <div
              ...
              <div
                className="stat-yearly-change"
                style={{
                  fontSize: "24px",
                  fontWeight: "700",
                  color:
                    currentHistoryData.trend === "up"
                      ? "#28a745"
                      : currentHistoryData.trend === "down"
                        ? "#dc3545"
                        : "#6c757d",
                }}
              >
                {currentHistoryData.trend === "up" && "+"}
                {currentHistoryData.yearlyChange.toFixed(1)}%
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="performance-table">
            ...
            <tbody>
              {currentHistoryData.monthlyData.map((item, index) => {
                const prevValue =
                  index > 0
                    ? currentHistoryData.monthlyData[index - 1].value
                    : item.value;
                const change =
                  prevValue > 0
                  ...
                  <tr key={item.month}>
                    ...
                    <td>
                      <div style={{ fontWeight: "600" }}>
                        {currentHistoryData.prefix || ""}
                        {item.value.toLocaleString()} {currentHistoryData.unit}
                      </div>
                    </td>
                    <td>
  ...
  const stats = useMemo(
    () => calculateStats(),
    [alPerformance, apPerformance, mpStats],
  );

  // Use real chart data from backend (mpStats) - MEMOIZED
  const mostAvailedPolicies = useMemo(
    () => mpStats?.policyDistribution || [],
    [mpStats?.policyDistribution],
  );
  const monthlyIssuedPolicies = useMemo(
    () => mpStats?.monthlyTrend || [],
    [mpStats?.monthlyTrend],
  );
  ...
  const monthlyChartData = useMemo(
    () => ({
      ...
      datasets: [
        {
          label: "Total Cases",
          data: monthlyIssuedPolicies.map(
            (m) => (m.issued || 0) + (m.declined || 0),
          ),
          backgroundColor: "#003781",
          borderRadius: 6,
        },
  ...
  if (error) {
    return (
      <div className="mp-dashboard-content" style={{ padding: "24px" }}>
        ...
        <div className="dashboard-grid">
          ...
          >
            <div className="stat-header">
              <div className="stat-label">
                {months[appliedFilters.month]} ANP
              </div>
              <div
                className={`stat-trend ${formatStatTrend("monthlyANP").className}`}
              >
                {formatStatTrend("monthlyANP").arrow}{" "}
                {formatStatTrend("monthlyANP").percentage}
              </div>
            </div>
            <div className="stat-value">
              ₱ {mpStats.monthlyANP.toLocaleString()}
            </div>
            <div className="stat-subtext">{selectedMonthYear} Performance</div>
            <Sparkline
              data={
                mpStats.monthlyTrend?.map((m) => m.anp).slice(-6) || [
                  100, 200, 150, 300, 250,
                ]
              }
              color="#60a5fa"
          ...
          >
            <div className="stat-header">
              ...
              <div
                className={`stat-trend ${formatStatTrend("totalCases").className}`}
              >
                {formatStatTrend("totalCases").arrow}{" "}
                {formatStatTrend("totalCases").percentage}
              </div>
            </div>
            <div className="stat-value">
              {(
                mpStats.monthlyCases + (mpStats.monthlyDeclined || 0)
              ).toLocaleString()}
            </div>
            <div className="stat-subtext">Policies (Issued + Declined)</div>
            <Sparkline
              data={
                mpStats.monthlyTrend?.map(
                  (m) => (m.issued || 0) + (m.declined || 0),
                ) || [2, 5, 3, 8, 4]
              }
              color="#f59e0b"
        ...
        <div className="dashboard-grid">
          ...
          >
            <div className="stat-header">
              ...
              <div
                className={`stat-trend ${formatStatTrend("totalALs").className}`}
              >
                {formatStatTrend("totalALs").arrow}{" "}
                {formatStatTrend("totalALs").percentage}
              </div>
            </div>
            <div className="stat-value">{mpStats.totalALs}</div>
            <div className="stat-subtext">
              {stats.performingALs} Performing (
              {mpStats.totalALs > 0
                ? ((stats.performingALs / mpStats.totalALs) * 100).toFixed(0)
                : 0}
              %)
            </div>
            <Sparkline
              data={[5, 6, 6, 7, 7, mpStats.totalALs]}
              color="#8b5cf6"
            />
          </div>
          ...
          >
            <div className="stat-header">
              ...
              <div
                className={`stat-trend ${formatStatTrend("totalAPs").className}`}
              >
                {formatStatTrend("totalAPs").arrow}{" "}
                {formatStatTrend("totalAPs").percentage}
              </div>
            </div>
            <div className="stat-value">{mpStats.totalAPs}</div>
            <div className="stat-subtext">
              {stats.activeAPs} Active (
              {mpStats.totalAPs > 0
                ? ((stats.activeAPs / mpStats.totalAPs) * 100).toFixed(0)
                : 0}
              %)
            </div>
            <Sparkline
              data={[10, 12, 15, 14, 18, mpStats.totalAPs]}
              color="#ec4899"
            />
          </div>
          ...
          >
            ...
            <div className="stat-value">
              {stats.avgActivityRatio.toFixed(1)}%
            </div>
            <div className="stat-subtext">Average across all APs</div>
            <Sparkline
              data={[12, 14, 13, 15, 14, stats.avgActivityRatio]}
              color="#14b8a6"
            />
          </div>
          ...
          >
            ...
            <div className="stat-value">
              ₱{" "}
              {stats.avgANPPerAP.toLocaleString(undefined, {
            ...
            <div className="chart-container animate-spring delay-4">
              <div className="chart-header">
                <div className="chart-title">
                  Monthly Issued Policies - {appliedFilters.year}
                </div>
                ...
                {topPerformers.map((al, index) => {
                  return (
                    <tr key={al.id}>
                      ...
                      <td>
                        <div className="agent-info">
                          ...
                          <div className="agent-detail">
                            ID: AL-{al.id.toString().padStart(4, "0")}
                          </div>
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        <span
                          className={`status-badge status-${al.status?.toLowerCase().replace(" ", "-") || "active"}`}
                        >
                          {al.status || "Active"}
                        </span>
                      </td>
    ...
    <>
      ...
      {showStatDetailsModal && (
        <div className="mp-modal">
          <div
            className="mp-modal-content animate-spring"
      ...
      {showAPsModal && selectedAL && (
        <div className="mp-modal">
          ...
          >
            ...
            <div className="mp-modal-body">
              <div
                className="mp-modal-summary-card"
                style={{
              ...
              <h4
                className="mp-modal-section-title"
              ...
              <table className="performance-table">
                ...
                <tbody>
                  {apPerformance
                    ...
                    .slice(0, 5)
                    .map((ap) => {
                      const performanceStatus = getAPPerformanceStatus(
                        ap.monthlyCases,
                      );
                        ...
                        <tr key={ap.id}>
                          <td>
                            <div className="agent-info">
                              ...
                              <div className="agent-detail">
                                ID: AP-{ap.id.toString().padStart(4, "0")}
                              </div>
                            </div>
                          </td>
                          <td>
                            <span
                              className={`status-badge status-${performanceStatus.toLowerCase().replace(" ", "-")}`}
                            >
                              {performanceStatus}
                            </span>
                          </td>
                          ...
                          <td>
                            <button
                              onClick={() =>
                                navigate(`/mp/ap-performance?ap=${ap.id}`)
                              }
                              className="details-button"
                            >
                              View Details
          ...
          >
            <div className="mp-modal-header">
              <div>
                ...
                >
                  Policy distribution and monthly performance for{" "}
            ...
            <div className="mp-modal-body">
                  ...
                  >
                    <div>
                      ...
                      <div style={{ height: "300px" }}>
                        <Bar
                          data={{
                            labels: policyDetailsData.policyDistribution.map(
                              (p) => p.policy_name,
                            ),
                            datasets: [
                              {
                                label: "Policy Count",
                                data: policyDetailsData.policyDistribution.map(
                                  (p) => p.count,
                                ),
                                backgroundColor: [
                                  "#003781",
                    ...
                    <div>
                      ...
                      >
                        <h3
                          ...
                          style={{ margin: 0 }}
                        >
                          Monthly Trend - {appliedFilters.year}
                        </h3>
                            ...
                            datasets: [
                              {
                                ...
                                data: policyDetailsData.monthlyTrend.map(
                                  (m) => m.policiesIssued,
                                ),
                                backgroundColor: "#003781",
                                borderRadius: 6,
              ...
              >
                <h4
                  ...
                  style={{ marginBottom: "16px" }}
                >
                  Policy Statistics - {selectedMonthYear}
                </h4>
                <div
                  style={{
                    display: "grid",
