...
const APPerformance = () => {
  ...
  const [apTeamData, setApTeamData] = useState([]);
  const [loadingApTeam, setLoadingApTeam] = useState(false);

  // Policy Details Data for AP
  ...
  useEffect(() => {
    if (showAPDetailsModal && selectedAP) {
      setLoadingApTeam(true);
      setLoadingPolicyDetails(true);

      // 1. Fetch Team (Recruits)
      fetch(
        `http://localhost:3000/api/mp/ap-details/${selectedAP.id}?year=${appliedFilters.year}&month=${appliedFilters.month}`,
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.success) setApTeamData(data.data);
          else setApTeamData([]);
        })
        .catch((err) => console.error("Error fetching AP team:", err))
        .finally(() => setLoadingApTeam(false));

      // 2. Fetch Policy Details (Personal + Team Production)
      fetch(
        `http://localhost:3000/api/mp/policy-details/${selectedAP.id}?year=${appliedFilters.year}`,
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.success) setPolicyDetailsData(data.data);
        })
        .catch((err) => console.error("Error fetching policy details:", err))
        .finally(() => setLoadingPolicyDetails(false));
    }
  }, [
    showAPDetailsModal,
    selectedAP,
  ...
  const fetchMonthlyANPTrend = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data.monthlyTrend) {
        ...
        const anpData = result.data.monthlyTrend.map((m) =>
          Math.round(m.anp / 1000),
        );
  ...
  const performingAPs = filteredAPs.filter(
    (ap) => getAPPerformanceStatus(ap.monthlyCases) === "PERFORMING",
  ).length;
  const totalANP = filteredAPs.reduce((sum, ap) => sum + ap.totalANP, 0);
  const monthlyANP = filteredAPs.reduce((sum, ap) => sum + ap.monthlyANP, 0);
  const avgMonthlyCases =
    totalAPs > 0
      ? filteredAPs.reduce((sum, ap) => sum + ap.monthlyCases, 0) / totalAPs
      : 0;
  ...
  useEffect(() => {
    if (showStatDetailsModal && selectedStat) {
      ...
      if (["totalANP", "totalCases"].includes(backendStatType)) {
        fetchStatHistoryData(backendStatType).then((data) =>
  ...
  const renderStatDetails = () => {
        ...
        case "totalAPs":
          return (
            <div>
              ...
              <p>
                Showing detailed information about all Agency Partners in the
                network.
              </p>
              <div className="info-grid">
                ...
                <div className="info-item">
                  ...
                  <span className="info-value">
                    {totalAPs > 0
                      ? ((activeAPs / totalAPs) * 100).toFixed(1)
                      : 0}
                    %
                  </span>
                </div>
        ...
        case "activeAPs":
          return (
            <div>
              ...
              <p>
                Agency Partners who have issued at least 1 policy this month.
              </p>
    ...
    const monthlyIssuedSum = filteredAPs.reduce(
      (sum, ap) => sum + ap.monthlyCases,
      0,
    );
    const monthlyDeclinedSum = filteredAPs.reduce(
      (sum, ap) => sum + (ap.monthlyDeclined || 0),
      0,
    );
              ...
              >
                {selectedStat === "avgMonthlyCases" ? (
                  <>
                    ...
                    >
                      {monthlyIssuedSum.toLocaleString()} Issued ·{" "}
                      {monthlyDeclinedSum.toLocaleString()} Declined
                    </div>
                  </>
                ) : (
    ...
    <>
      ...
      <div className="dashboard-grid">
        ...
        >
          ...
          <div className="stat-subtext">
            {totalAPs > 0 ? ((activeAPs / totalAPs) * 100).toFixed(1) : 0}%
            Active Rate
          </div>
        </div>
        ...
        >
          ...
          <div className="stat-value">
            {(
              filteredAPs.reduce((sum, ap) => sum + ap.monthlyCases, 0) +
              filteredAPs.reduce(
                (sum, ap) => sum + (ap.monthlyDeclined || 0),
                0,
              )
          ...
          >
            {filteredAPs
              .reduce((sum, ap) => sum + ap.monthlyCases, 0)
              .toLocaleString()}{" "}
            Issued ·{" "}
            {filteredAPs
              .reduce((sum, ap) => sum + (ap.monthlyDeclined || 0), 0)
              .toLocaleString()}{" "}
            Declined
          </div>
        </div>
          ...
          <div className="chart-container">
            <div className="chart-header">
              ...
              <div className="chart-subtitle">
                Performance levels across all APs
              </div>
            </div>
                  ...
                  datasets: [
                    {
                      data: [
                        filteredAPs.filter(
                          (ap) =>
                            getAPPerformanceStatus(ap.monthlyCases) ===
                            "PERFORMING",
                        ).length,
                        filteredAPs.filter(
                          (ap) =>
                        ...
                        filteredAPs.filter(
                          (ap) =>
                            getAPPerformanceStatus(ap.monthlyCases) ===
                              "NEEDS IMPROVEMENT" && ap.monthlyCases > 0,
                        ).length,
                        filteredAPs.filter((ap) => ap.monthlyCases === 0)
                          .length,
                      ],
                      backgroundColor: [
                        "#28a745",
          ...
          <div className="chart-container">
            <div className="chart-header">
              <div className="chart-title">
                Monthly ANP Trend - {appliedFilters.year}
              </div>
      ...
      <div className="content-container">
        <div className="container-header">
          <h2>
            Agency Partners Detailed View - {months[appliedFilters.month]}{" "}
            {appliedFilters.year}
          </h2>
          <div className="card-header-stats">
            <span className="stat-badge">
              Showing: {filteredAPs.length} of {apPerformance.length}
            </span>
            <span className="stat-badge status-active">
              Active: {activeAPs}
            </span>
            <span className="stat-badge status-performing">
              Performing: {performingAPs}
            </span>
          </div>
        </div>
                  ...
                  <tr key={ap.id}>
                    ...
                    <td>
                      <div className="activity-status-cell">
                        <span
                          className={`activity-dot ${activityStatus.toLowerCase()}`}
                        ></span>
      ...
      {showAPDetailsModal && selectedAP && (
        <div className="mp-modal">
          <div className="mp-modal-content" style={{ maxWidth: "1000px" }}>
            ...
            <div className="mp-modal-body">
              {loadingPolicyDetails ? (
                ...
                <>
                      ...
                      <div>
                        ...
                        >
                          {policyDetailsData.policyDistribution.length > 0
                            ? policyDetailsData.policyDistribution[0]
                                .policy_name
                            : "N/A"}
                        </div>
                      </div>
                  ...
                  <div
                    className="modal-table-responsive"
                        ...
                        {apTeamData.map((agent) => (
                          <tr key={agent.id}>
                            ...
                            <td style={{ color: "#28a745", fontWeight: "600" }}>
                              ₱ {agent.monthlyANP.toLocaleString()}
                            </td>
                            <td>
                              <div style={{ fontWeight: "600" }}>
                                {agent.monthlyCases}
                              ...
                              >
                                {agent.monthlyDeclined || 0} Declined
                              </div>
                            </td>
                            <td>
                              <span
                                className={`status-badge status-${agent.status.toLowerCase().replace(" ", "-")}`}
                              >
                                {agent.status}
                              </span>
                            </td>
