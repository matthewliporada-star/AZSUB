...
const ALPerformance = () => {
  ...
  const { alPerformance, apPerformance, getAPsByAL, loading, refreshData } =
    useMPData();
  ...
  const filteredALs = (alPerformance || []).filter((al) => {
    ...
    if (
      ...
      al.status !== appliedFilters.status
    ) {
      return false;
    }
  ...
  const performingALs = filteredALs.filter(
    (al) => al.status === "PERFORMING",
  ).length;
  const averageActivityRatio =
    filteredALs.length > 0
      ? filteredALs.reduce((sum, al) => sum + al.activityRatio, 0) /
        filteredALs.length
      : 0;
  ...
  const getAPPerformanceSummaryForAL = (alName) => {
    ...
    const performing = aps.filter(
      (ap) => getAPPerformanceStatus(ap.monthlyCases) === "PERFORMING",
    ).length;
    const average = aps.filter(
      (ap) => getAPPerformanceStatus(ap.monthlyCases) === "AVERAGE",
    ).length;
    const needsImprovement = aps.filter(
      (ap) => getAPPerformanceStatus(ap.monthlyCases) === "NEEDS IMPROVEMENT",
    ).length;
  ...
  const getAdjustedMonthlyPerformance = (al) => {
    ...
    const monthMultiplier =
      appliedFilters.month === 11
        ? 1.3
        : appliedFilters.month === 0
          ? 0.8
          : 1.0;
    ...
    return {
      policiesIssued: Math.round(
        basePerformance.policiesIssued * totalMultiplier,
      ),
  ...
  const renderStatDetails = () => {
        ...
        case "totalALs":
          return (
            <div>
              ...
              <p>
                Showing detailed information about all Agency Leaders in the
                network.
              </p>
              <div className="info-grid">
                ...
                <div className="info-item">
                  ...
                  <span className="info-value">
                    {filteredALs.filter((al) => al.status === "AVERAGE").length}
                  </span>
                </div>
                <div className="info-item">
                  ...
                  <span className="info-value">
                    {
                      filteredALs.filter(
                        (al) => al.status === "NEEDS IMPROVEMENT",
                      ).length
                    }
                  </span>
                </div>
          ...
          return (
            <div>
              ...
              <table className="performance-table">
                ...
                <tbody>
                  {filteredALs
                    .filter((al) => al.status === "PERFORMING")
                    .map((al) => (
                      <tr key={al.id}>
                        <td>{al.name}</td>
    ...
    const monthlyIssuedSum = filteredALs.reduce(
      (sum, al) => sum + al.monthlyCases,
      0,
    );
    const monthlyDeclinedSum = filteredALs.reduce(
    ...
    <>
      ...
      <div className="dashboard-grid">
        ...
        >
          ...
          <div className="stat-subtext">
            {totalALs > 0 ? ((performingALs / totalALs) * 100).toFixed(1) : 0}%
            of total
          </div>
        </div>
        ...
        >
          ...
          <div className="stat-value">
            {(
              filteredALs.reduce((sum, al) => sum + al.monthlyCases, 0) +
              filteredALs.reduce(
                (sum, al) => sum + (al.monthlyDeclined || 0),
                0,
              )
          ...
          >
            {filteredALs
              .reduce((sum, al) => sum + al.monthlyCases, 0)
              .toLocaleString()}{" "}
            Issued ·{" "}
            {filteredALs
              .reduce((sum, al) => sum + (al.monthlyDeclined || 0), 0)
              .toLocaleString()}{" "}
            Declined
          </div>
        </div>
      ...
      <div className="content-container">
        <div className="container-header">
          <h2>
            Agency Leaders Detailed View - {months[appliedFilters.month]}{" "}
            {appliedFilters.year}
          </h2>
          <div className="card-header-stats">
            <span className="stat-badge">
              Showing: {filteredALs.length} of {alPerformance.length}
            </span>
            <span className="stat-badge status-performing">
              Performing: {performingALs}
            </span>
          </div>
        </div>
                  ...
                  <tr key={al.id}>
                    <td>
                      <div className="agent-info">
                        ...
                        <div className="agent-detail">{al.city}</div>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <div style={{ fontSize: "12px", color: "#64748b" }}>
                        {apSummary.active} active (
                        {apSummary.active > 0
                          ? Math.round(
                              (apSummary.active / apSummary.total) * 100,
                            )
                          : 0}
                        %)
                      </div>
                    ...
                    <td>
                      <div style={{ fontWeight: "600" }}>
                        {monthlyPerformance.policiesIssued +
                          (al.monthlyDeclined || 0)}
                      </div>
                    </td>
                    <td>
                      <span
                        className={`status-badge status-${al.status.toLowerCase().replace(" ", "-")}`}
                      >
                        {al.status}
                      </span>
                    </td>
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {Math.floor(
                        getAPsByAL(selectedAL.name).reduce(
                          (sum, ap) => sum + (ap.monthlyCases || 0),
                          0,
                        ) / getAPsByAL(selectedAL.name).length,
                      )}
                    </div>
                  </div>
                </div>
                        ...
                        >
                          ...
                          >
                            {policyDetailsData.totalCases +
                              policyDetailsData.monthlyTrend.reduce(
                                (sum, m) => sum + (m.declined || 0),
                                0,
                              )}
                          </span>
                        </div>
                            ...
                            datasets: [
                              {
                                ...
                                data: policyDetailsData.monthlyTrend.map(
                                  (m) => m.policiesIssued + (m.declined || 0),
                                ),
                                backgroundColor: "#003781",
                                borderRadius: 6,
                              },
